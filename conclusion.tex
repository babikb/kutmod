...
This empirical study conducted a rigorous evaluation of leading Large Language Models (LLMs)—including models from the Gemini, GPT, and DeepSeek families—specifically assessing their capabilities in Automated Program Repair (APR) when confronted with the inherent challenges of concurrent programming problems. The evaluation focused on three critical dimensions: \textbf{Patch Accuracy}, \textbf{Newly Introduced Bugs (Side Effects)}, and \textbf{Efficiency} \cite{anand2024}.

%ide kell a kovi graph 
\begin{figure}
    \centering
    \includegraphics[width=0.7\columnwidth]{test/new_bugcount.pdf}
    \caption*{
        New bugs created by each LLM. 
    }
\end{figure}

The differential between different Large Language Models concerning the introduction of new bugs is largely indistinguishable as the data visualized above shows. As it is illustrated, the data is mostly stagnant and not of concern with regards to our final conclusions.

With regards to newly introduced bugs, the concept of a measure of severity can be brought up. As defined in this paper, the severity of a bug can be described as the effort needed to be put in to be able to solve it. With that being said though, this metric is dubious, vaguely defined and hard to measure. "Effort" as a quantitative resource is hard to define as first we need to ascertain who the final completion of the work will be done by. Human programmers and other APRs might find different errors harder or easier to fix not to even talk about the difference between those categories. Surface level metrics such as amount of lines changed can act as indicators but these metrics can at best work as guesstimation and a haruspex might provide better results.
